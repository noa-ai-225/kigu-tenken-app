...
if mode == "📋 現場点検入力":
    ...
    # --- 【復活】全体備考欄 ---
    st.divider()
    memo = st.text_area("📝 備考（異常の詳細や未実施の理由などを記入）")
    ...
    if st.button("このラインの点検結果を送信", type="primary", use_container_width=True):
        ...
        for equipment, final_status in equip_results.items():
            # 写真ファイル名の生成（※クラウド上ではファイル自体は一時保存）
            photo_name = ""
            if photo_files[equipment]:
        ...
        st.success("スプレッドシートへ備考・判定を保存しました！")
        st.balloons()

# --- 2. 設備項目設定 / 3. 履歴確認 モード（変更なし） ---
elif mode == "🛠 設備項目設定":
    st.title("設備項目設定")
    df_master = load_master()
    ...
    try:
        ...
        def style_rows(row):
            res_str = str(row.結果)
            if "❌NG" in res_str: return ['background-color: #d00000; color: white; font-weight: bold'] * len(row)
            if "⚠️未実施" in res_str: return ['background-color: #ff8c00; color: black; font-weight: bold'] * len(row)
            return [''] * len(row)
        
        st.dataframe(df_res.sort_values(by="日付", ascending=False).style.apply(style_rows, axis=1), use_container_width=True)
    except:
        st.info("データがありません。")
